    ...
    while True:
        ...
        if(curFrame % step ==0):
            if frameChecked == 0: # init
                previousFrame = np.zeros((tmpFrame.shape[0], tmpFrame.shape[1], 1)).astype(np.uint8)
                previousFrameF = np.zeros((tmpFrame.shape[0], tmpFrame.shape[1], 1)).astype(np.float32)
                currentFrame  = np.zeros((tmpFrame.shape[0], tmpFrame.shape[1], 1)).astype(np.uint8)
                currentFrameF = np.zeros((tmpFrame.shape[0], tmpFrame.shape[1], 1)).astype(np.float32)
                tmpFrameF = np.zeros((tmpFrame.shape[0], tmpFrame.shape[1], 1)).astype(np.float32)
                frameChecked += 1
            else:
                currentFrame = cv2.cvtColor(tmpFrame, cv2.COLOR_RGB2GRAY)
                tmpFrameF = copy.deepcopy(currentFrame).astype(np.float32)
                previousFrameF = copy.deepcopy(previousFrame).astype(np.float32)
    
                currentFrameF = np.abs(tmpFrameF - previousFrameF)
                currentFrame = copy.deepcopy(currentFrameF).astype(np.uint8)
                currentFrame = cv2.threshold(currentFrame, 30, 255, cv2.THRESH_BINARY)[1]
                g_nStructElementSize = 3
                element = cv2.getStructuringElement(cv2.MORPH_RECT, (2 * g_nStructElementSize + 1, 2 * g_nStructElementSize + 1), (g_nStructElementSize, g_nStructElementSize))
